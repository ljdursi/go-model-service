...
The installation of this particular project is verbose so as to offer a tutorial on which aspects of the server backend are auto-generated by tools in the stack, and which files act as configuration to this auto-generation step.
You can learn more about the code-generating tools employed here in the [For Developers](#for-developers) section of this README.
...
### Installing the Go-Model-Service
...
1. Now that all the necessary boilerplate code has been auto-generated, compile the server by running:
  ```
  $ cd $GOPATH/src/github.com/CanDIG/go-model-service/model-vs/api
  $ go build -tags sqlite cmd/variant-service-server/main.go
  ```
2. Run the server on a port of your choosing (eg. port 3000):
  ```
  $ cd $GOPATH/src/github.com/CanDIG/go-model-service/model-vs/api
  $ ./main --port=3000
  ```
