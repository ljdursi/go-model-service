...
// NewServer creates a new api variant service server but does not configure it
func NewServer(api *operations.VariantServiceAPI) *Server {
	s := new(Server)
...
// Server for the variant service API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/variant-service.sock"`
	...
	api          *operations.VariantServiceAPI
...
func (s *Server) SetAPI(api *operations.VariantServiceAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
