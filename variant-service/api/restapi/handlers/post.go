...
func PostCall(params operations.PostCallParams) middleware.Responder {
	...
	_, err := getCallByID(params.Call.ID.String(), tx)
	if err == nil { // TODO this is not a great check
	...
	newCall, errPayload := transformations.CallAPIToDataModel(*params.Call, tx)
	if errPayload != nil {
		return operations.NewPostCallInternalServerError().WithPayload(errPayload)
	}
	...
	// so it should be deleted prior to return
	retrievedDataCall, err := getCallByID(newCall.ID.String(), tx)
	if err != nil {
		errors.Log(err, 500, funcName,
	...
	retrievedAPICall, errPayload := transformations.CallDataToAPIModel(*retrievedDataCall)
	if err != nil {
		return operations.NewPostCallInternalServerError().WithPayload(errPayload)
	}
