...
func variantDataToAPIModel(dataVariant datamodels.Variant) (*apimodels.Variant, *apimodels.Error) {
	funcName := "handlers.variantDataToAPIModel"

	apiVariant, err := transformers.VariantDataToAPI(dataVariant)
	if err != nil {
		errors.Log(err, 500, funcName+", transformers.VariantDataToAPI",
			"Failed transformation of Variant from data to api model")
		errPayload := errors.DefaultInternalServerError()
		return nil, errPayload
	}
...
func variantAPIToDataModel(apiVariant apimodels.Variant, tx *pop.Connection) (*datamodels.Variant, *apimodels.Error) {
	funcName := "handlers.variantAPIToDataModel"

	dataVariant, err := transformers.VariantAPIToData(apiVariant)
	if err != nil {
		errors.Log(err, 500, funcName+", transformers.VariantAPIToData",
			"Failed transformation of Variant from api to data model")
		errPayload := errors.DefaultInternalServerError()
		return nil, errPayload
	}
...
func callDataToAPIModel(dataCall datamodels.Call) (*apimodels.Call, *apimodels.Error) {
	funcName := "handlers.callDataToAPIModel"

	apiCall, err := transformers.CallDataToAPI(dataCall)
	if err != nil {
		errors.Log(err, 500, funcName+", transformers.CallDataToAPI",
			"Failed transformation of Call from data to api model")
		errPayload := errors.DefaultInternalServerError()
		return nil, errPayload
	}
...
func callAPIToDataModel(apiCall apimodels.Call, tx *pop.Connection) (*datamodels.Call, *apimodels.Error) {
	funcName := "handlers.callAPIToDataModel"

	dataCall, err := transformers.CallAPIToData(apiCall)
	if err != nil {
		errors.Log(err, 500, funcName+", transformers.CallAPIToData",
			"Failed transformation of Call from api to data model")
		errPayload := errors.DefaultInternalServerError()
		return nil, errPayload
	}
